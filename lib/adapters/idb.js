...
function init(api, opts, callback) {
  ...
  // between attachment digests and docId+revs
  var ATTACH_AND_DOCIDREV_STORE = 'attach-docidrev-store';
  ...
  function createSchema(db) {
    ...
    // added in v4
    var attAndRevStore = db.createObjectStore(ATTACH_AND_DOCIDREV_STORE,
      {autoIncrement: true});
    attAndRevStore.createIndex('doc_id_rev', 'doc_id_rev');
    attAndRevStore.createIndex('digest', 'digest');
  }
  ...
  function addAttachAndDocIdRevStore(db) {
    var attAndRevStore = db.createObjectStore(ATTACH_AND_DOCIDREV_STORE,
      {autoIncrement: true});
    attAndRevStore.createIndex('doc_id_rev', 'doc_id_rev');
    attAndRevStore.createIndex('digest', 'digest');
  ...
  function migrateAttsAndDocIdRevs(txn) {
    var seqStore = txn.objectStore(BY_SEQ_STORE);
    var attStore = txn.objectStore(ATTACH_STORE);
    var attAndRevStore = txn.objectStore(ATTACH_AND_DOCIDREV_STORE);
    ...
    function writeDoc(docInfo, winningRev, deleted, callback, resultsIdx) {
      ...
      function insertAttachmentMappings(callback) {
        ...
        function add(att) {
          ...
          var req = txn.objectStore(ATTACH_AND_DOCIDREV_STORE).put({
            doc_id_rev: docIdRev,
            digest: digest
          });
  ...
  api._doCompaction = function (docId, rev_tree, revs, callback) {
    ...
    function deleteOrphanedAttachments() {
      ...
      possiblyOrphanedDigests.forEach(function (digest) {
        var countReq = attAndRevStore.index('digest').count(
    ...
    docStore.get(docId).onsuccess = function (event) {
      ...
      revs.forEach(function (rev) {
        ...
        index.getKey(key).onsuccess = function (e) {
          ...
          var cursor = attAndRevStore.index('doc_id_rev')
            .openCursor(global.IDBKeyRange.only(key));

          cursor.onsuccess = function (event) {
            var cursor = event.target.result;
            if (cursor) {
              possiblyOrphanedDigests.push(cursor.value.digest);
              attAndRevStore.delete(cursor.primaryKey);
              cursor.continue();
            } else { // done
              count--;
