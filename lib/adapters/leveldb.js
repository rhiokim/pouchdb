...
function LevelPouch(opts, callback) {
  ...
  api._doCompaction = function (docId, rev_tree, revs, callback) {
    ...
    if (db._compactionQueue.length === 1) {
      api._doCompactionSynchronously(docId, rev_tree, revs, callback);
    }
  };

  // execute compactions synchronously so we
  // can correctly dedup attachments
  api._doCompactionSynchronously = function (docId, rev_tree, revs, callback) {
    ...
    callback = function (err) {
      ...
      process.nextTick(function () {
        ...
        if (db._compactionQueue.length) {
          api._doCompactionSynchronously.apply(api, db._compactionQueue[0]);
        }
      });
    };
    if (!revs.length) {
